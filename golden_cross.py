...
class goldenCross:
    ...
    def validate_dataframe(self):
        ...
        if self.df.isnull().any().any():
            print("Warning: Data contains missing values. Filling forward/backward.")
...
def fetch_data_from_ib(symbol: str) -> pd.DataFrame:
    ...
    duration_string = "5 Y"
    bar_day_string = "1 day"

    print(f"[IB] Connecting to gateway on port 4002 with clientId=1 ...")
    ib.connect("127.0.0.1", 4002, clientId=1, timeout=10, readonly=False)

    if not ib.client.isReady():
        print("[IB] ERROR: Connection not ready. Check that TWS/Gateway is running on port 4002.")
        ib.disconnect()
        return stock_df

    print(f"[IB] Connected. Fetching 5Y daily data for {symbol} ...")
    try:
        ib.qualifyContracts(contract)
        bars = ib.reqHistoricalData(
            contract,
            endDateTime=end_date,
            durationStr=duration_string,
            barSizeSetting=bar_day_string,
            whatToShow='TRADES',
            useRTH=False,
            formatDate=1
        )
        ...
        if not bars:
            print(f"[IB] ERROR: No bars returned for {symbol}.")
            print("  Possible reasons:")
            print("  - Historical Data Farm is inactive in the gateway")
            print("  - Symbol not available in your IB data subscription")
            print("  - Market closed / outside trading hours for this exchange")
            ib.disconnect()
            return stock_df
        ...
        print(f"[IB] Data fetched successfully. Rows: {len(stock_df)}")

    except Exception as e:
        print(f"[IB] Exception while fetching data: {e}")
    finally:
        ib.disconnect()
...
def test_strategy(df: pd.DataFrame, initial_capital: float):
    """Long-short strategy: goes long on buy signal, short on sell signal."""
...
def test_strategy_long_only(df: pd.DataFrame, initial_capital: float):
    """Long-only strategy: buys on cross-up, sells on cross-down, never shorts."""
    starting_capital = initial_capital
    ...
    print("📈 Long-Only Strategy Backtest Results:")
    print(f"  Initial Capital  : ₹{starting_capital:,.2f}")
    print(f"  Final Capital    : ₹{final_capital:,.2f}")
    print(f"  Sharpe Ratio     : {sharpe_ratio:.2f}")
    print(f"  Sortino Ratio    : {sortino_ratio:.2f}")
...
if __name__ == "__main__":
    ...
    if stock_df.empty:
        print("\n[ERROR] No data retrieved from Interactive Brokers. Backtest cannot run.")
        print("  - Ensure IBKR Gateway/TWS is running on port 4002")
        print("  - Ensure the Historical Data Farm is active in the gateway")
        print("  - Ensure SBIN is a valid contract on NSE with currency INR")
        exit(1)
